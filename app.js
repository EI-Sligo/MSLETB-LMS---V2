...
const schedulerManager = {
    ...
    // --- UNIT MENU (Now inside the object!) ---
    editSlot: (slot) => {
        const existing = document.getElementById('menu-modal'); if(existing) existing.remove();
        const modal = document.createElement('div');
        modal.id = 'menu-modal';
        ...
        modal.innerHTML = `
            <div class="bg-white rounded-lg shadow-2xl w-full max-w-sm overflow-hidden p-4 space-y-3 border border-gray-200">
                ...
                <div class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Global Shift (From ${new Date(dateStr).toLocaleDateString()})</div>
